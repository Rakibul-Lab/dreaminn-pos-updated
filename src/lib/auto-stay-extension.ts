...
/** Guests may check out until 2:00 PM on departure day; after that an extra night is added. */
export const AUTO_EXTENSION_GRACE_END_TIME = '14:00'

export function getAutoExtensionCutoff(checkOut: Date, now: Date = new Date()): Date {
  return applyHotelTimeToDate(startOfDay(checkOut), AUTO_EXTENSION_GRACE_END_TIME)
}

export function isPastAutoExtensionCutoff(checkOut: Date, now: Date = new Date()): boolean {
  return now.getTime() > getAutoExtensionCutoff(checkOut, now).getTime()
}
...
export async function extendOverdueCheckedInBooking(
  ...
  while (isPastAutoExtensionCutoff(checkOut, now) && extensions < 60) {
  ...
  await db.booking.update({
    ...
    data: {
      ...
      dueAmount,
      notes: booking.notes
        ? `${booking.notes}\nAuto-extended ${extensions} night(s) after 2:00 PM checkout grace.`
        : `Auto-extended ${extensions} night(s) after 2:00 PM checkout grace.`,
    },
  })
...
export async function processAllOverdueStayExtensions(
  ...
  for (const booking of checkedIn) {
    if (!isPastAutoExtensionCutoff(booking.checkOut, now)) continue
    const didExtend = await extendOverdueCheckedInBooking(db, booking.id, now)
